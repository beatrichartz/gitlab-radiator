...
async function fetchLatestAndMasterPipeline(projectId, config) {
  ...
  const latestPipeline = _.take(pipelines, 3)
  if (latestPipeline[0].ref === 'master') {
    return latestPipeline
  }
  const latestMasterPipeline = _(pipelines).filter({ref: 'master'}).take(3).value()
  ...
  return latestPipeline.concat(_.take(masterPipelines, 3))
}
